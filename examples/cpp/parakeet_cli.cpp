...
void print_usage(const char* program_name) {
    ...
    std::cout << "  --model  <name>      Model variant: parakeet-v2 or parakeet-v3 (default: parakeet-v2)\n";
...
int main(int argc, char* argv[]) {
    ...
    // Allow selecting model via flag or env var EDDY_PARAKEET_MODEL
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            if (i + 1 >= argc) {
                std::cerr << "Error: --model requires an argument (parakeet-v2 or parakeet-v3)\n";
                return 1;
            }
            model_name = argv[++i];
        } else {
    ...
    try {
        ...
        // For v3, require v3-specific vocab; else use standard set
        if (!eddy::parakeet::check_models_available(cache_model_dir, &fetch_err,
            (model_name == "parakeet-v3" ? eddy::model_configs::PARAKEET_V3_FILES
        ...
        eddy::parakeet::ModelPaths paths{
            ...
            .tokenizer_json = (model_name == "parakeet-v3")
                                ? (model_dir / "parakeet_v3_vocab.json").string()
                                : (model_dir / "parakeet_vocab.json").string()
        };

        // Configure runtime (v2 uses blank_token_id=1024; v3 uses 8192)
        eddy::parakeet::RuntimeConfig cfg{
            .device = device,
            .blank_token_id = (model_name == "parakeet-v3") ? 8192 : 1024,
